...
type GetDashboardRow struct {
	...
	ImportsNeedingReview int64 `json:"imports_needing_review"`
}

func (q *Queries) GetDashboard(ctx context.Context) (GetDashboardRow, error) {
	row := q.db.QueryRow(ctx, getDashboard)
	var i GetDashboardRow
	err := row.Scan(
		&i.Assets,
		&i.Liabilities,
		&i.Buckets,
		&i.ImportsNeedingReview,
	)
	return i, err
}
